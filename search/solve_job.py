...
class SolveJob():
    ...
    def execute(self):
        solver = self.solver_class(
            network=self.network, metric=self.metric, n_actions=self.n_actions, shuffles=self.shuffles)
    ...
    def log_results(self, results):
        for log_num, result in enumerate(results):
            ...
            # self.loggers.log_scalar('solution_length', self.step, len(
            #     result['solution']) if result['solution'] is not None else -1)
            # self.loggers.log_scalar('nodes', self.step, result['tree_metrics']['nodes'])
            # self.loggers.log_scalar('expanded_nodes', self.step,
            #                         result['tree_metrics']['expanded_nodes'])
        ...
        self.loggers.log_scalar(get_name('solved_rate'), self.step,
                                self.solved_boards / self.all_boards)

        avg_length = 0 if not self.solution_lengths else (
            sum(self.solution_lengths)/len(self.solution_lengths))
        self.loggers.log_scalar(get_name('avg_length'), self.step, avg_length)

        for budget in self.budget_checkpoints:
            self.loggers.log_scalar(get_name(
                f'solved_rate_{budget}_nodes'), self.step, self.budget_solved[budget] / self.all_boards)
            self.loggers.log_scalar(get_name(
                f'solved_rate_{budget}_exp_nodes'), self.step, self.budget_exp_solved[budget] / self.all_boards)
